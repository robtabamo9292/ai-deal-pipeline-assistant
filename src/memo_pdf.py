...
def _split_long_word(
    ...
    """Split unusually long tokens so no text extends beyond the page."""
    chunks: list[str] = []
    current = ""
    for char in word:
...
def _priority_color(priority: str):
    ...
    if "high" in normalized or "advance" in normalized or "priority" in normalized:
        return GREEN
...
def _draw_metric_card(
    ...
    pdf.setFillColor(_priority_color(clean_value) if label == "PRIORITY" else NAVY)
    pdf.setFont("Helvetica-Bold", font_size)
    pdf.drawCentredString(x + width / 2, y + 13, clean_value)


def memo_to_pdf_bytes(memo: Any) -> bytes:
    """Create a concise, one-page investment brief PDF from an InvestmentMemo."""
    buffer = BytesIO()
    ...
    pdf.roundRect(MARGIN_X, header_y, CONTENT_WIDTH, header_height, 10, fill=1, stroke=0)
    ...
    title_size = _fit_font_size(company_name, "Helvetica-Bold", 17, 11, title_width)
    pdf.setFillColor(WHITE)
    pdf.setFont("Helvetica-Bold", title_size)
    pdf.drawString(title_x, header_y + 45, company_name)

    subtitle = f"{_clean(memo.sector)} | {_clean(memo.subsector)} | {_clean(memo.stage)}"
    _draw_wrapped(
        pdf,
        subtitle,
        title_x,
    ...
    pdf.roundRect(MARGIN_X, callout_y, CONTENT_WIDTH, callout_height, 9, fill=1, stroke=1)
    pdf.setFillColor(BLUE)
    pdf.setFont("Helvetica-Bold", 9.2)
    pdf.drawString(MARGIN_X + 14, callout_y + callout_height - 19, "RECOMMENDED NEXT STEP")
    _draw_wrapped(
        pdf,
        recommendation,
        MARGIN_X + 14,
