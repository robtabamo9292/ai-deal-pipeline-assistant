...
from src.ui.components import clean_text, render_metrics, render_result
...
with st.sidebar:
    st.markdown(
        ...
        <div class="sidebar-brand">
          ...
          <div class="sidebar-sub">Private-market diligence workspace</div>
        </div>
        """,
    ...
    st.markdown(
        ...
        "5. Export a deal record or brief"
    )
    st.divider()
...
st.caption(
    ...
    "diligence questions, CRM-ready data, and an investment brief."
)

tab_deal, tab_pipeline, tab_memo = st.tabs(
    ["Deal Intake", "Pipeline", "Investment Brief"]
)
...
with tab_deal:
    ...
    with right:
        ...
        if latest is None:
            ...
            st.markdown(f"**Suggested demo:** {sample_preview}")
        else:
...
with tab_pipeline:
    ...
    st.caption(
        "Analyzed deals remain in the current browser session and can be "
        "exported as CSV."
    )
    if not st.session_state.deals:
        st.info("No analyzed companies yet.")
    else:
        dataframe = create_pipeline_dataframe(st.session_state.deals)
        display_columns = [
            "company_name",
            "sector",
            "stage",
...
with tab_memo:
    st.markdown(
        '<div class="section-title">Investment brief</div>',
        unsafe_allow_html=True,
    )
    st.caption(
        "Generate a one-page analyst brief with methodology and "
        "provenance metadata."
    )
    if not st.session_state.deals:
        st.info("Analyze a deal before generating a brief.")
    else:
        names = [
            clean_text(deal.company_name, f"Deal {index + 1}")
            for index, deal in enumerate(st.session_state.deals)
        ]
        ...
        selected_index = st.selectbox(
            ...
            index=max(0, min(default_index, len(names) - 1)),
            format_func=lambda index: names[index],
        )
        selected_deal = st.session_state.deals[selected_index]
        selected_notes = st.session_state.deal_inputs[selected_index]
        ...
        st.download_button(
            f"Download brief for {names[selected_index]}",
            data=pdf_bytes,
